...
class RelativeMultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        relative_attn_weights: Optional[Tensor] = None

        if need_weights:
            relative_attn_weights = attn_weights_float.view(
                bsz, self.num_heads, tgt_len, src_len
            ).transpose(1, 0)
            if not need_head_weights:
                # average attention weights over heads
                relative_attn_weights = relative_attn_weights.mean(dim=0)
