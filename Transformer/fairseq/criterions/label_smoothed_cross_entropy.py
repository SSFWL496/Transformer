...
class LabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def compute_loss(self, model, net_output, sample, reduce=True, teacher_output=None, distil_strategy="normal", update_num=None):
        ...
        if distil_strategy == 'normal' or teacher_output is None:
            ...
            KL_loss = F.kl_div(lprobs, distil_lprobs, reduction='none')  #compute the Kullback-Leibler divergence Loss
            KL_loss = KL_loss.sum(dim=-1)    #sum the batch 
            KL_loss.masked_fill_(pad_mask, 0.)    #delete the mask word
            KL_loss = KL_loss.sum()   #sum the sentence
            extra_result['KD_loss'] = KL_loss  #add the kd_loss
            loss = golden_loss + KL_loss   #the translation loss + kl_loss
        elif distil_strategy == 'distil_only':
            # only get supervision signal from KD   #only use the kd loss
            distil_lprobs = self.get_teacher_probs(teacher_output)
            ...
            loss_gate = nll_loss.topk(math.ceil(words_num * word_rate), dim=0, largest=True)[0][-1]  #"word rate" is the rate for selected word number
            KL_mask = nll_loss < loss_gate
            KL_loss = F.kl_div(lprobs, distil_lprobs, reduction='none')
            KL_loss = KL_loss.sum(dim=-1)
            KL_loss = KL_loss.view(-1)
            KL_loss = KL_loss[~pad_mask]
            KL_loss = KL_loss[~KL_mask]     #only use the part word to kd
            KD_loss = KL_loss.sum() 
            ...
            # from the queue get the gate    #it is the same for translation memory
